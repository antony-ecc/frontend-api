        ...
        <div v-if="post">
            <div class="border-l-4 border-blue-500 pl-4 mt-12">
                ...
                <div v-if="authStore.user && authStore.user.id === post.user_id" class="flex items-center gap-6 mt-6">
                    ...
                    <router-link :to="{ name: 'update', params: { id:post.id } }" class="text-green-500 font-bold px-2 py-1 border border-green-300">Update</router-link>
                </div>
            </div>
        </div>
        <div v-else>
            <p class="title">Post not found</p>
        </div>
    </main>
</template>